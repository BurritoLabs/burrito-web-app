...
- Maintains bounded owner and LP-token indexes for lock queries.
- Supports an in-place, batched index migration for existing deployments.
...
The repository includes a GitHub Actions workflow pinned to the Rust version
used by the CosmWasm optimizer:
...
For a first deployment, use `npm run deploy:launchpad`. For an existing
deployment, use `npm run upgrade:launchpad`; it preserves lock records and
finishes the secondary-index migration before reporting success. See
`docs/launchpad-deploy.md` for the exact dry-run and confirmation commands.
